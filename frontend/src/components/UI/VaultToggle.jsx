...
const VaultToggle = ({ onClick, documentCount = 0 }) => {
  return (
    ...
    >
      <FolderOpen className="w-5 h-5 text-gray-600 dark:text-gray-300 group-hover:text-[#D97706] dark:group-hover:text-amber-400 transition-colors" />
      {documentCount > 0 && (
        <span className="absolute -top-1 -right-1 bg-[#D97706] text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">
